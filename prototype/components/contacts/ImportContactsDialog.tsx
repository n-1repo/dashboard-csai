...
import { useStore, type ContactImportMode } from "@/lib/store";
...
export function ImportContactsDialog({ contacts }: ImportContactsDialogProps) {
  ...
  function handleConfirm() {
    const outcome = importContacts(processed.dedupedRows, mode);
    ...
    setResult({
      ...
      invalid: processed.invalidRows.length,
      duplicates_in_file: processed.duplicatesInFile,
      invalid_rows: processed.invalidRows,
    });
    setStep("result");
  }
      ...
      <DialogContent className="max-w-2xl">
        ...
        {step === "preview" ? (
          <div className="flex flex-col gap-4">
            ...
            <div className="flex justify-end gap-2">
              <Button variant="outline" type="button" onClick={() => setStep("map")}>
                Kembali
              </Button>
              <Button type="button" onClick={handleConfirm} disabled={processed.dedupedRows.length === 0}>
                Confirm Import
              </Button>
            </div>
          </div>
